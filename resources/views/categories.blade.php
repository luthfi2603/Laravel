@extends('layouts.main')

@section('container')
    <h1 class="mb-4">Post Categories</h1>
    <div class="row">
        @foreach($categories as $category)
            <div class="col-md-4">
                <a href="/posts?category={{ $category->slug }}">
                    <div class="card text-bg-dark">
                        <img src="
                        @php
                            if($category->name === "Web Programming"){
                                echo"http://localhost:8000/assets/img/web-programing3.jpg";
                            }else if($category->name === "Web Design"){
                                echo"http://localhost:8000/assets/img/web-design3.jpg";
                            }else{
                                echo"http://localhost:8000/assets/img/personal3.jpg";
                            }
                        @endphp
                        " class="card-img" alt="{{ $category->name }}">
                        <div class="card-img-overlay d-flex align-items-center p-0">
                            <h5 class="card-title text-center flex-fill p-4 fs-3" style="background-color: rgba(0, 0, 0, 0.7);">{{ $category->name }}</h5>
                        </div>
                    </div>
                </a>
            </div>
        @endforeach
    </div>
@endsection